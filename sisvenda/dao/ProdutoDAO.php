...
class ProdutoDAO
{
    ...
    public function listar($nome)
    {
        ...
        try {
            ...
            $pdo = Conexao::connect();
            $sql = "SELECT id,nome, preco, descricao,idcategoria FROM produto where nome like :nome";
    ...
    public function listarUm($id)
    {
        try {
            $categoriaDAO= new CategoriaDAO();
            $pdo = Conexao::connect();
            $sql = "SELECT id, nome, preco, descricao, idcategoria FROM produto where id = :id";
            $consulta = $pdo->prepare($sql);
            $consulta->execute(array(
                ':id' => $id
            ));
    ...
    public function listarTodos()
    {
        try {
            ...
            while ($linha = $consulta->fetch(PDO::FETCH_ASSOC)) {
                $produto = new Produto( $linha['nome'], $linha['descricao'], $linha['preco']);
                $produto->setId($linha['id']);
                $produto->setCategoria($categoriaDAO->listarUm($idcategoria));
                $produtos[] = $produto;
            }
        } catch (PDOException $e) {
            echo 'Error: ' . $e->getMessage();
